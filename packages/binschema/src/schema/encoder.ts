...
function encodeString(stringField: any, value: string): number[] {
  ...
  if (encoding === 'ascii') {
    stringBytes = Array.from(value).map(c => c.charCodeAt(0) & 0x7F);
  } else if (encoding === 'latin1') {
    // Latin-1 is a 1:1 byte mapping (0x00-0xFF maps to U+0000-U+00FF)
    stringBytes = Array.from(value).map(c => c.charCodeAt(0) & 0xFF);
  } else {
    // UTF-8 (default)
    stringBytes = Array.from(new TextEncoder().encode(value));
  }
