...
const ComputedFieldSchema = z.object({
  ...
  from_after_field: z.string().optional().meta({
...
const ArrayKindSchema = z.enum([
  ...
  "packed_count",    // Thrift packed header: (count<<4)|element_type_tag, 0xF escape + LEB128
]);
...
const ArrayElementSchema = z.object({
  ...
  element_type_tag: z.number().int().min(0).max(15).optional(), // For packed_count: Thrift element type id (low nibble)
...
const ArrayFieldSchema = z.object({
  ...
  length_type: z.enum(["uint8", "uint16", "uint32", "uint64", "varlength"]).optional(), // For length_prefixed and byte_length_prefixed
...
function validateTerminalVariants(schema: any): { valid: boolean; error?: string } {
  ...
  for (const [typeName, typeDef] of Object.entries(schema.types)) {
    ...
    const checkArray = (arrayDef: any, path: string) => {
      ...
      // terminal_variants only makes sense for null_terminated or variant_terminated arrays
