...
export function generateZig(
  ...
  const usesRoot = schemaUsesRootDecode(schema);
  ...
  for (const [name, typeDef] of Object.entries(schema.types)) {
    ...
    if ("sequence" in (typeDef as any)) {
      lines.push(...generateStructCode(name, typeDef as any, schema, defaultEndianness, defaultBitOrder, rootTypeNameZ));
...
function generateStructCode(
  ...
  const emit: EmitCtx = { schema, endianness, bitOrder, selfPath: "self", fields, selfTypeName: name, rootTypeName };
