...
export interface ContextRequirements {
  ...
  // Uses back_reference fields (needs shared compression dict and absolute positions)
  usesBackReferences: boolean;
}
...
export function analyzeContextRequirements(
  ...
  // Check if this type definition itself is a back_reference
  ...
  for (const field of fields) {
    ...
    // Handle sum_of_sizes and sum_of_type_sizes which use "targets" (plural)
    ...
    // Check for back_reference fields (need shared compression dict)
    if (fieldAny.type === 'back_reference') {
      requirements.usesBackReferences = true;
    }

    // Check if this is a type reference to a back_reference type
    if (fieldAny.type && typeof fieldAny.type === 'string' && schema.types[fieldAny.type]) {
      const referencedType = schema.types[fieldAny.type] as any;
...
export function generateContextInterface(schema: BinarySchema): string {
  ...
  // Add back_reference support (compression dict and absolute byte offset tracking)
