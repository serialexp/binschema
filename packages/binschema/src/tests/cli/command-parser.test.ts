...
function expectError(argv: string[], substring: string): void {
  ...
  assert(errorResult.error.message.includes(substring), `Expected error to include "${substring}" but was "${errorResult.error.message}"`);
}
...
export function runCommandParserTests(): { passed: number; failed: number; checks: TestCheck[] } {
  ...
  expectError(["generate", "--schema", "schema.json", "--out", "./gen"], "Missing required option: --language <ts|go|rust|python|zig>");
