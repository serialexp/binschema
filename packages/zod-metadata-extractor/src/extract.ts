...
function extractFieldInfo(
  ...
  // Determine if field is required by checking the def type
  // Note: Don't use isOptional() as it triggers sync parse which fails for promise types
  const required = fieldDef?.type !== "optional";
...
function getTypeName(fieldDef: any): string {
  ...
  // Handle enum types - distinguish regular enum from nativeEnum
  if (type === "enum") {
    // Check if this is a nativeEnum by looking at the entries
    // nativeEnum entries have numeric keys (from reverse mapping of TS enums)
    if (fieldDef?.entries) {
      const entries = fieldDef.entries;
      const keys = Object.keys(entries);
      // Check if any key is a number (indicates nativeEnum with numeric values)
  ...
  // Handle union types - distinguish regular union from discriminatedUnion
  if (type === "union") {
  ...
  // Handle pipe types - in Zod 4, transform/preprocess/pipe are all "pipe"
  // We can try to distinguish them by looking at the structure
  if (type === "pipe") {
    // Check if the 'in' is a transform or has preprocess characteristics
    // For now, just return "pipe" - more detailed analysis would require deeper inspection
    return "pipe";
  }

  // For array, return just "array" (element info is extracted separately)
  if (type === "array") {
    return "array";
  }
...
export function walkUnion(
  ...
  for (const optionSchema of def.options) {
    ...
    // Check if this option itself is a union (regular or discriminated)
    const isNestedUnion = optDef?.type === "union" || optDef?.discriminator;
    if (isNestedUnion && optDef?.options) {
      // Recursively walk this nested union
      ...
      if (meta) {
        // Determine the key: use discriminator value if available, otherwise fall back to title
        let key = discriminatorValue;
        if (!key && meta.title) {
          // Convert title to snake_case key (e.g., "Conditional Field" -> "conditional_field")
