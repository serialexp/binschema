...
describe("Zod Type Coverage Analysis", () => {
  ...
  describe("Complex Types", () => {
    ...
    test("function", () => {
      // Zod 4 uses .input() and .output() instead of .args() and .returns()
  ...
  describe("Transformations and Refinements", () => {
    ...
    test("transform", () => {
      // In Zod 4, transforms are represented as "pipe" internally
      const schema = z.object({
        value: z.string().transform(s => s.length)
      });
      const fields = extractFields(schema);
      expect(fields![0].type).toBe("pipe");
    });

    test("preprocess", () => {
      // In Zod 4, preprocess is represented as "pipe" internally
      const schema = z.object({
        value: z.preprocess(
          (val) => String(val),
          z.string()
  ...
  describe("Pipeline (Zod 4)", () => {
    test("pipe", () => {
      // In Zod 4, pipe type is simply "pipe"
      const schema = z.object({
