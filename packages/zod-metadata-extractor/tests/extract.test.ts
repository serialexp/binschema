...
describe("walkUnion", () => {
  ...
  test("extracts metadata using title when no discriminator field", () => {
    const schema = z.union([
      z.object({
        // No "type" field - but has metadata with title
        value: z.string(),
      }).meta({
        title: "No Discriminator",
        description: "A type without a discriminator field",
      }),
    ]);

    const result = walkUnion(schema);

    // Should extract using title as key (converted to snake_case)
    expect(result.hasMetadata).toBe(true);
    expect(result.metadata.size).toBe(1);
    expect(result.metadata.has("no_discriminator")).toBe(true);
    expect(result.metadata.get("no_discriminator")?.title).toBe("No Discriminator");
  });
